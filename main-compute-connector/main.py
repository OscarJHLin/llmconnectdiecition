#!/usr/bin/env python3
"""
NetConnect Main Compute Node - 连接已有Hermes版本
通过WebSocket或API连接外部Hermes Agent，带Web UI管理控制台
运行方式: python main.py
"""
...
_app_state = {
    ...
    "config": {
        ...
        "port": 11434,
        "model": "",
        "node_name": "main-compute-connector",
        "host": "0.0.0.0",
        "port_network": 8888,
        "multicast_group": "224.0.0.1",
        "multicast_port": 5000,
        "hermes_url": "http://localhost:8000",
        "hermes_ws": "ws://localhost:8000/ws",
        "feishu": {"enabled": False, "app_id": "", "app_secret": ""},
        "weixin": {"enabled": False, "token": ""},
    },
    "status": {"node_running": False, "ai_connected": False, "hermes_connected": False},
    "logs": [],
    "chat_history": [],
    "tasks": [],
}
...
class MainNetworkNode:
    ...
    async def start(self):
        ...
        _log(f"Main network node started on {self.host}:{self.port}")
        async with self._server:
            await self._server.serve_forever()

class MainComputeConnector:
    def __init__(self, node_name="main-connector"):
        self.node_name = node_name
        self.tasks = {}
        self.hermes_connected = False

    async def connect_hermes(self, url):
        """连接外部Hermes Agent"""
        try:
            import requests
            ...
            if response.status_code == 200:
                ...
                _log(f"Connected to Hermes Agent at {url}")
                return True
        except Exception as e:
            _log(f"Failed to connect Hermes: {e}")
        return False
    ...
    async def get_status(self):
        return {
            ...
            "hermes_connected": self.hermes_connected,
        }
...
<title>NetConnect Main Compute - Connector</title>
<style>
...
.connector-info { background: rgba(166,227,161,0.1); border: 1px solid var(--success); padding: 16px; border-radius: 8px; margin-bottom: 16px; }
</style>
</head>
<body>
<div class="container">
  <div class="sidebar">
    ...
    <button onclick="showTab('connector')">Hermes连接</button>
  ...
  <div class="main">
    ...
    <div id="connector" class="tab-content">
      <div class="connector-info">
        <h3>外部 Hermes Agent 连接</h3>
        <p>此版本通过 API/WebSocket 连接已有的 Hermes Agent 实例</p>
        <div class="status-grid" style="margin-top:16px;">
          <div class="status-card">
            <div class="label">连接状态</div>
            <div class="value" id="hermesConnStatus">未连接</div>
          </div>
        </div>
      </div>
      <div class="form-group">
        <label>Hermes Agent URL</label>
        <input type="text" id="hermesUrl" value="http://localhost:8000" placeholder="http://localhost:8000">
      </div>
      <div class="form-group">
        <label>WebSocket URL</label>
        <input type="text" id="hermesWs" value="ws://localhost:8000/ws" placeholder="ws://localhost:8000/ws">
      </div>
      <button class="btn btn-primary" onclick="connectHermes()">连接 Hermes</button>
    </div>
    ...
    <div id="settings" class="tab-content">
      ...
      <div class="form-group">
        <label>节点名称</label>
        <input type="text" id="nodeName" value="main-compute-connector">
      </div>
      <div class="form-group">
        <label>网络端口</label>
...
async function connectHermes() {
  const data = {
    url: document.getElementById('hermesUrl').value,
    ws: document.getElementById('hermesWs').value
  };
  const result = await api('connect_hermes', data);
  if (result.ok) {
    document.getElementById('hermesConnStatus').textContent = '已连接';
    alert('Hermes Agent 连接成功');
  } else {
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _process_action(self, action, data):
        ...
        elif action == "connect_hermes":
            return self._do_connect_hermes(data)
        return {"error": "unknown action"}

    def _do_start_node(self, data):
        try:
            node_name = data.get("node_name") or "main-compute-connector"
            network_port = int(data.get("network_port", 8888))

            compute = MainComputeConnector(node_name=node_name)
            network = MainNetworkNode(port=network_port)
            ...
            _log("Main compute connector starting...")
    ...
    def _do_connect_hermes(self, data):
        compute = _app_state.get("compute_node")
        if not compute:
            return {"ok": False, "error": "Compute node not running"}
        try:
            url = data.get("url", "http://localhost:8000")
            future = _runner.run(compute.connect_hermes(url))
            result = future.result(timeout=10)
            if result:
                _app_state["status"]["hermes_connected"] = True
                _app_state["config"]["hermes_url"] = url
                return {"ok": True}
...
def main():
    ...
    print(f"=" * 60)
    print(f"  NetConnect Main Compute Node - Connector")
    print(f"=" * 60)
