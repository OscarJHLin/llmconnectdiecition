#!/usr/bin/env python3
"""
NetConnect Central Network Node - 独立安装包
仅命令行运行，无需Web UI
...
async def main(port, multicast_group, multicast_port):
    ...
    print("=" * 60)
    print("  NetConnect Central Network Router")
    print("=" * 60)
    print(f"  TCP Port: {port}")
    print(f"  Multicast: {multicast_group}:{multicast_port}")
    print("=" * 60)
    log("Starting central network router...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NetConnect Central Network Router")
