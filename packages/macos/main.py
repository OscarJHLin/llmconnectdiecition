...
class ModelAdapter:
    def __init__(self, backend: int, base_url: str, port: int):
...
<title>NetConnect Node Installer - macOS</title>
<style>
:root {
  --bg: #1e1e2e;
  --fg: #cdd6f4;
...
.hidden { display: none; }
#scanResult { margin-top: 16px; padding: 12px; background: rgba(0,0,0,0.2); border-radius: 8px; }
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('nodeName').value = 'macos-node-' + Math.random().toString(36).substr(2, 6);
});
</script>
</body>
</html>
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    async def _run_hermes_chat(self, hermes, text):
        try:
