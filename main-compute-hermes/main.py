#!/usr/bin/env python3
"""
NetConnect Main Compute Node - 自带Hermes Agent版本
包含完整Hermes Agent功能，带Web UI管理控制台
运行方式: python main.py
"""
...
class MainComputeNode:
    def __init__(self, node_name="main"):
        self.node_name = node_name
        self.tasks = {}
        self.hermes_initialized = False
    ...
    async def get_status(self):
        return {
            ...
            "hermes_available": self.hermes_initialized,
        }
...
<title>NetConnect Main Compute - Hermes</title>
<style>
...
.hermes-info { background: rgba(137,180,250,0.1); border: 1px solid var(--accent); padding: 16px; border-radius: 8px; margin-bottom: 16px; }
</style>
</head>
<body>
<div class="container">
  ...
  <div class="main">
    ...
    <div id="hermes" class="tab-content">
      <div class="hermes-info">
        <h3>Hermes Agent 状态</h3>
        <p>此版本自带完整 Hermes Agent 功能</p>
        <div class="status-grid" style="margin-top:16px;">
          <div class="status-card">
            <div class="label">Agent状态</div>
            <div class="value" id="hermesStatus">未初始化</div>
          </div>
          <div class="status-card">
            <div class="label">记忆系统</div>
            <div class="value" id="memoryStatus">离线</div>
          </div>
          <div class="status-card">
      ...
      <button class="btn btn-primary" onclick="initHermes()">初始化 Hermes Agent</button>
    </div>
    ...
    <div id="settings" class="tab-content">
      ...
      <div class="form-group">
        <label>节点名称</label>
        <input type="text" id="nodeName" value="main-compute">
      </div>
      <div class="form-group">
        <label>网络端口</label>
...
async function initHermes() {
  const result = await api('init_hermes');
  if (result.ok) {
    document.getElementById('hermesStatus').textContent = '已初始化';
    document.getElementById('memoryStatus').textContent = '在线';
    alert('Hermes Agent 初始化成功');
  }
}
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _process_action(self, action, data):
        ...
        elif action == "init_hermes":
            return self._do_init_hermes()
        return {"error": "unknown action"}

    def _do_start_node(self, data):
        try:
            node_name = data.get("node_name") or "main-compute"
            network_port = int(data.get("network_port", 8888))

            compute = MainComputeNode(node_name=node_name)
            ...
            _log("Main compute node starting...")
    ...
    def _do_init_hermes(self):
        compute = _app_state.get("compute_node")
        if compute:
            compute.hermes_initialized = True
            _log("Hermes Agent initialized")
            return {"ok": True}
        return {"ok": False, "error": "Compute node not running"}
...
def main():
    ...
    print(f"=" * 60)
    print(f"  NetConnect Main Compute Node - Hermes")
    print(f"=" * 60)
