#!/usr/bin/env python3
"""
NetConnect Android Sub Node - Android子节点
包含网络子节点和计算子节点功能，带Web UI（移动端优化）
运行方式: python main.py
"""
...
_app_state = {
    ...
    "config": {
        ...
        "backend_port": 11434,
        "model": "",
        "node_name": f"android-node-{uuid.uuid4().hex[:6]}",
...
<title>NetConnect Android Sub Node</title>
<style>
:root { --bg: #121212; --fg: #e0e0e0; --accent: #bb86fc; --success: #03dac6; --warning: #ffab40; --error: #cf6679; --surface: #1e1e1e; --border: #2c2c2c; }
* { box-sizing: border-box; }
body { margin: 0; font-family: "Roboto", system-ui, sans-serif; background: var(--bg); color: var(--fg); font-size: 14px; }
...
.step { background: var(--surface); border-radius: 12px; padding: 16px; margin-bottom: 12px; }
.step-title { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
.step-number { width: 28px; height: 28px; border-radius: 50%; background: var(--accent); color: #000; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 14px; }
.step h2 { margin: 0; color: var(--fg); font-size: 16px; }
...
.btn { padding: 12px 20px; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 500; width: 100%; margin-bottom: 8px; }
...
.status-card .label { font-size: 11px; color: #a0a0a0; }
.status-card .value { font-size: 16px; font-weight: bold; margin-top: 4px; }
.tab-container { display: flex; gap: 4px; margin-bottom: 12px; }
.tab { padding: 8px 12px; border: none; border-radius: 8px; background: var(--surface); color: var(--fg); cursor: pointer; font-size: 13px; }
.tab.active { background: var(--accent); color: #000; }
.tab-content { display: none; }
.tab-content.active { display: block; }
.chat-container { background: var(--surface); border-radius: 8px; overflow: hidden; }
.chat-messages { height: 250px; overflow-y: auto; padding: 12px; }
.chat-message { margin-bottom: 10px; }
.chat-message.user .msg-content { background: var(--accent); color: #000; }
.chat-message.assistant .msg-content { background: var(--border); }
.msg-content { padding: 8px 12px; border-radius: 16px; max-width: 85%; font-size: 14px; }
...
.chat-input button { padding: 10px 16px; background: var(--accent); color: #000; border: none; border-radius: 8px; cursor: pointer; }
.log-panel { background: #0a0a0a; padding: 10px; border-radius: 8px; height: 180px; overflow-y: auto; font-family: monospace; font-size: 11px; white-space: pre-wrap; }
.scan-result { margin-top: 12px; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 8px; font-size: 13px; }
</style>
</head>
<body>
<div class="container">
  ...
  <div class="step">
    ...
    <span id="nodeStatus" class="badge error" style="margin-left: 8px;">已停止</span>
  </div>
...
def main():
    ...
    print(f"=" * 60)
    print(f"  NetConnect Android Sub Node")
    print(f"=" * 60)
