#!/usr/bin/env python3
"""
NetConnect Linux Sub Node - Linux子节点
...
_app_state = {
    ...
    "config": {
        ...
        "backend_port": 11434,
        "model": "",
        "node_name": f"linux-node-{uuid.uuid4().hex[:6]}",
...
<title>NetConnect Linux Sub Node</title>
<style>
:root { --bg: #1a1a2e; --fg: #eaeaea; --accent: #e94560; --success: #0f3460; --warning: #e94560; --error: #e94560; --surface: #16213e; --border: #0f3460; }
* { box-sizing: border-box; }
body { margin: 0; font-family: "Ubuntu", "DejaVu Sans", system-ui, sans-serif; background: var(--bg); color: var(--fg); }
...
.badge.success { background: rgba(15,52,96,0.5); color: #fff; }
...
.status-card .label { font-size: 12px; color: #a0a0a0; }
...
.log-panel { background: #0a0a0a; padding: 12px; border-radius: 8px; height: 200px; overflow-y: auto; font-family: "Ubuntu Mono", monospace; font-size: 12px; white-space: pre-wrap; }
...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('nodeName').value = 'linux-node-' + Math.random().toString(36).substr(2, 6);
});
</script>
</body>
</html>
...
def main():
    ...
    print(f"=" * 60)
    print(f"  NetConnect Linux Sub Node")
    print(f"=" * 60)
