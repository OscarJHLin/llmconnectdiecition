#!/usr/bin/env python3
"""
NetConnect macOS Sub Node - macOS子节点
...
_app_state = {
    ...
    "config": {
        ...
        "backend_port": 11434,
        "model": "",
        "node_name": f"macos-node-{uuid.uuid4().hex[:6]}",
...
<title>NetConnect macOS Sub Node</title>
<style>
:root { --bg: #1e1e2e; --fg: #cdd6f4; --accent: #89b4fa; --success: #a6e3a1; --warning: #f9e2af; --error: #f38ba8; --surface: #313244; --border: #585b70; }
...
def main():
    ...
    print(f"=" * 60)
    print(f"  NetConnect macOS Sub Node")
    print(f"=" * 60)
